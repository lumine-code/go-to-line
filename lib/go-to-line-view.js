...
class GoToLineView {
  constructor() {
    this.inputDialogView = lumine.workspace.buildInputDialog({
      ...
      commands: {
        "go-to-line:navigate": {
          description: "Move the active editor to the entered row and column.",
    ...
    this.inputDialogView.onDidChangeQuery(() => this.navigate({ keepOpen: true }));
    ...
    // line in.
    lumine.commands.add("lumine-workspace", "go-to-line:toggle", () => {
      this.toggle();
      return false;
    });
  }
  ...
  open() {
    if (this.inputDialogView.isVisible() || !lumine.workspace.getActiveTextEditor()) return;
